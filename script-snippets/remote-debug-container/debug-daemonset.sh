...
if [[ "$@" == *"RESTORE_BACKUP"* ]]; then
    if [[ ! -z "$(kubectl get configmap remote-debug-backup-$DS_NAME -o Name || true)" ]]; then
        kubectl delete secret local-ssh-keys 2> /dev/null || true
        kubectl get configmap remote-debug-backup-$DS_NAME -o 'jsonpath={.data}' > $DS_JSON_FILE
        ...
        CONTAINER_IDX=$(jq -rc '.container_idx' $DS_JSON_FILE)

        RESTORE_PATCH_JSON=$(echo "$RESTORE_PATCH" | \
            sed "s;CONTAINER_IDX;$CONTAINER_IDX;" | \
            sed "s;VOLUMES_JSON;$(jq -rc '.deploy_json | fromjson | .spec.template.spec.volumes' $DS_JSON_FILE);" | \
            sed "s;SECURITY_CONTEXT_JSON;$(jq -rc '.deploy_json | fromjson | .spec.template.spec.securityContext' $DS_JSON_FILE);" | \
            sed "s;CONTAINER_JSON;$(jq -rc ".deploy_json | fromjson | .spec.template.spec.containers[$CONTAINER_IDX]" $DS_JSON_FILE);" | \
        jq -c)

        kubectl patch daemonset $DS_NAME --type json --patch "$RESTORE_PATCH_JSON"
        kubectl delete configmap remote-debug-backup-$DS_NAME
        exit 0
    fi

    >&2 echo "No backup for daemonset '$DS_NAME' found"
    exit 1
fi


CONTAINER_JSON=
kubectl get daemonset $DS_NAME -o json > $DS_JSON_FILE
if [[ -z "$(head -n 1 $DS_JSON_FILE)" ]]; then
    >&2 echo "DaemonSet '$DS_NAME' not found"
    exit 1
fi

if [[ -z "$CONTAINER_NAME" ]]; then
    CONTAINER_JSON=$(jq -rc ".spec.template.spec.containers[0] //empty" $DS_JSON_FILE)
else
    CONTAINER_JSON=$(jq -rc ".spec.template.spec.containers[] | select(.name==\"$CONTAINER_NAME\") //empty" $DS_JSON_FILE)
fi

if [[ -z "$CONTAINER_JSON" ]]; then
    >&2 echo "Container '$CONTAINER_NAME' not found in daemonset $DS_NAME"
    exit 1
fi
CONTAINER_NAME=$(echo $CONTAINER_JSON | jq -rc '.name')
CONTAINER_IMAGE=$(echo $CONTAINER_JSON | jq -rc '.image')
CONTAINER_IDX=$(jq -rc ".spec.template.spec.containers | map(.name == \"$CONTAINER_NAME\") | index(true)" $DS_JSON_FILE)
...
EXISTING_RUNAS_NONROOT=$(cat $DS_JSON_FILE | jq -rc '.spec.template.spec.securityContext.runAsNonRoot //empty')
EXISTING_VOLUMES=$(cat $DS_JSON_FILE | jq -rc '.spec.template.spec.volumes //empty')
...
if [[ -z "$(kubectl get configmap remote-debug-backup-$DS_NAME -o Name || true)" ]]; then
    kubectl create configmap remote-debug-backup-$DS_NAME --from-file deploy_json=$DS_JSON_FILE --from-literal container_idx=$CONTAINER_IDX
fi

# cat $PATCH_JSON
kubectl patch daemonset $DS_NAME --type json --patch-file "$PATCH_JSON"
